...
The trait the rules implement (the `try_` variants default to these):
...
Rules emit new operations through the builder:
...
JVP rules also receive `primal_outputs`: a rule whose derivative depends on the
output reuses it instead of recomputing. For example, `exp`'s JVP is
`d(exp x) = exp(x) · dx`, so the rule references the primal output directly.
