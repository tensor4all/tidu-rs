...
**Architecture:** Keep the existing generic trace algorithm, but hide trace node and edge internals behind an opaque `Trace<Op>`. Route recording through `Recorder`, route backward through `try_backward`, and expose only the downstream runtime hooks needed for concrete replay and transpose execution.
...
Update eager tests so they import `tidu::eager::{...}` and `tidu::emit::try_transpose_fragment` instead of root eager symbols.
...
Move `try_eager_transpose_fragment` to `emit::try_transpose_fragment` and drop the panic wrapper.

- [ ] **Step 2: Run emitter-focused tests**

Run: `cargo nextest run --release --test eager_backward_tests emit_try_transpose_fragment_fan_out_accumulation`
...
- [ ] **Step 2: Add `BackwardExecutor` and `try_backward`**

Move the current reverse traversal behind `try_backward`, with trace sorting internal to the module.
